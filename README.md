...
- **Difficulties encountered**: Some entries in the `occurrences.csv` have datatype properties that would be applicable to the associated `dcterms:Location`, but none for the associated `dwc:Event`. There may be different reasons for this:
  ...
  In some cases, the original image is apparently the only variant, but is the value entered for both the `accessURI` and `goodQualityAccessURI` entries. This would cause issues for the object property `dwcdp:derivedFrom`, which would end up stating that a media instance was derived from itself. This required verification not only that entries were present for the variants, but also that they were different from the original image.

  A final difficulty was with dates, which showed various issues. Some rows that considered the value of `0` for temporal fields. For example, 1 entry has `dwc:year` entered as `0`, 49 entries have `dwc:month` entered as `0`, and 434 entries have `dwc:day` entered as `0`. Inspection of some of these specimen with digitized images revealed that these are often used as placeholders for when the entry is unavailable. For example, [this specimen of *Cibotium chamissoi*](https://s.idigbio.org/idigbio-images-prod-webview/5c452b766f539460d0e8249873afe79c.jpg), which has a year, but no month. In this case, the entry of `0` for `dwc:month` was removed, rather than considering an incorrect value.
  
  The issue is further complicated by entries such as [this specimen of *Clermontia persicifolia*](https://pacific.symbiota.org/media/HAW_/HAW11/HAW11602_1764514908_web.jpg), which clearly has a monthly value, but has a `dwc:month` entry of `0`. Even the entries show `dwc:eventDate` as `1959-09` and `dwc:verbatimEventDate` as `Nov., 1959`. Consequently, there should be some level of verification between the entries in `dwc:eventDate`, preferably entered in an ISO 8601 compliant format, and the integer entries in `dwc:year`, `dwc:month` and `dwc:day`.
  
  Finally, [this specimen of *Bidens pilosa*](https://pacific.symbiota.org/media/HAW_/HAW08/HAW08659_1_1764565080_web.jpg) is noteworthy. The month is inintelligible, bu we know it was collected on the 24th of some month in 1965. The entry for `dwc:month` is entered as `0`, again as a placeholder for unknown value. However, the issue is that the `dwc:eventDate` associated with it is `1965-24`, which is incorrect by ISO 8601 standards. No validity checks were done during writing of the turtle file, as the entry was `"1965-24"^^xsd:gYearMonth`, though the issue was raised during parsing of the file. What notified me of this issue was a regex pattern as `^\d{4}-(0[1-9]|1[0-2])$`, which picked up on the fact that the month entry was above the maximum possible value of 12.
